...
function SWEP:PrimaryAttack()
	...
	slash.endpos = self.Owner:GetShootPos() + self.Owner:GetAimVector() * self.HitDistance
	...
	if tr.Hit then	
		...
		if IsValid(tr.Entity) then
			...
			dmginfo:SetDamageForce(self.Owner:GetForward() * 400000 + self.Owner:GetAimVector() * 800000 + self.Owner:GetUp() * 200000)
			
			if SERVER then tr.Entity:TakeDamageInfo(dmginfo) end	
	...
	util.Decal("Scorch", tr.HitPos + tr.HitNormal, tr.HitPos - tr.HitNormal )	
	self.Owner:SetVelocity(420 * self.Owner:GetAimVector() * -1)
end
