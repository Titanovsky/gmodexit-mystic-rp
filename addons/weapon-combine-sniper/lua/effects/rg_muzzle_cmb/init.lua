...
function EFFECT:Init(data)
	...
	if self.WeaponEnt:GetOwner():IsValid() then
		self.AddVel = self.WeaponEnt:GetOwner():GetVelocity()
	else
		self.AddVel = Vector(0,0,0)
	end
	local emitter = ParticleEmitter(self.Position)
		
		local particle = emitter:Add("sprites/heatwave", self.Position - self.Forward*4)
		particle:SetVelocity(80*self.Forward + 20*VectorRand() + 1.05*self.AddVel)
	...
	for i=1,3 do 
		...
		particle:SetVelocity(30*i*self.Forward + 10*VectorRand() + 1.1*self.AddVel)
	...
	for i=1,4 do 
		local particle = emitter:Add("particle/particle_smokegrenade", self.Position)
		particle:SetVelocity(math.Rand(20,30)*self.Forward + 40*VectorRand() + 1.1*self.AddVel)
		particle:SetDieTime(math.Rand(0.36,0.38))
	...
	for i=1,2 do 
		...
		particle:SetVelocity(350*self.Forward + 1.1*self.AddVel)
