...
ENT.Class       = 'mystic_warehouse_take'
ENT.Type	    = 'anim'

ENT.PrintName	= 'DNS Take'
ENT.Author		= 'Ambi'
...
if CLIENT then
    ...
    function ENT:DrawTranslucent()
        ...
        if ( LocalPlayer():GetPos():Distance( self:GetPos() ) <= DISTANCE ) then
            ...
            Draw.Box( 330, 120, -160, -90, C.AMBI_BLOOD )
                Draw.Box( 330 - 4, 120 - 4, -160 + 2, -90 + 2, C.AMBI_BLACK )
                Draw.SimpleText( 2, -36, 'Сделать видеокарту [E]', UI.SafeFont( '38 Ambi' ), C.AMBI_WHITE, 'center', 1, C.ABS_BLACK )
            cam.End3D2D()
        end
    end
...
function ENT:Use( ePly )
    ...
    if ePly.has_videocard then return end

    local vcard = table.Random( videocards )

    ePly.has_videocard = vcard

    ePly:ChatSend( '~W~ Вы изготовили видеокарту: ~G~ '..vcard..' ~W~, отнесите её!' )
    ePly:EmitSound( 'ambi/ui/click3.ogg' )
    
    hook.Call( '[Ambi.MysticRP.TakeVideocard]', nil, ePly, vcard, self )
end
