...
ENT.Class       = 'npc_mystic_shop_perma_weapons'
ENT.Base	    = 'base_ai'
ENT.Type	    = 'ai'

ENT.PrintName	= 'NPC Show Perma Weapons'
ENT.Author		= 'Ambi'
...
ENT.Stats = {
    type = 'Entity',
    module = 'MysticRP',
    model = 'models/jrpc/jin.mdl',
    date = '31.08.2023 14:20'
}
...
if CLIENT then
    ...
    function ENT:DrawTranslucent()
        ...
        cam.Start3D2D( head + Vector( 0, 0, 12 ), Angle( 0, rot, 90 ), 0.1 )
            draw.SimpleTextOutlined( 'Продавец Perma Оружия', UI.SafeFont( '26 Ambi' ), 4, 0, C.AMBI_RED, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, C.ABS_BLACK )
        cam.End3D2D()
    end

    function Ambi.MysticRP.ShowShopPermaWeapons( nObj )
        ...
        for i, wep in ipairs( Ambi.MysticRP.Config.buyer_weapons ) do
            ...
            if not wep.perma then continue end
...
function ENT:Use( ePly )
    ePly:SendLua( 'Ambi.MysticRP.ShowShopPermaWeapons('..self:EntIndex()..')' )
end
