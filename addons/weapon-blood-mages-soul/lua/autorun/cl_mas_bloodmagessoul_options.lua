...
CreateConVar( 'sk_mas_bloodmagessoul_enabled', '1', { FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_SERVER_CAN_EXECUTE } )
CreateConVar( 'sk_mas_bloodmagessoul_adminonly', '0', { FCVAR_ARCHIVE, FCVAR_SERVER_CAN_EXECUTE } )
CreateConVar( 'sk_mas_bloodmagessoul_infinitecharge', '0', { FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_SERVER_CAN_EXECUTE }, "Enable infinite charge for this SWEP" )
CreateConVar( 'sk_mas_bloodmagessoul_nocooldown', '0', { FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_SERVER_CAN_EXECUTE }, "Disable cooldowns for this SWEP" )
...
CreateConVar( 'sk_mas_setmaxhealth_onspawn', '1', { FCVAR_ARCHIVE, FCVAR_REPLICATED, FCVAR_SERVER_CAN_EXECUTE }, "Leave this active unless it is actually conflicting with another addon. Disabling this results in healing abilities only going to 100 HP (the default max health) unless defined by another addon. DarkRP DOES NOT set MAX HEALTH by default, hence why I made this code!" )
	...
	Maranzos_AbilitySWEPs = Maranzos_AbilitySWEPs or {}
		...
		if SERVER then
			cvars.AddChangeCallback( "sk_mas_bloodmagessoul_infinitecharge", function( convar_name, value_old, value_new )
...
print("\n")
print("[MAS] Maranzo\'s Ability SWEPs: Blood Mage's Soul")
...
-- Tool Options
	...
	hook.Add( 'PopulateToolMenu', 'Maranzo_AS_BloodMagesSoul', function()
		spawnmenu.AddToolMenuOption( 'Options',
			'Ability SWEPs',
			'Maranzo_AS_BloodMagesSoul',
			"MAS: Blood Mage's Soul",
			'',
			'',
			function( pnl )
				pnl:ControlHelp( '' )
				pnl:Help( "Maranzo\'s Ability SWEPs: Blood Mage's Soul" )
				...
				btnBug.DoClick = function()
					gui.OpenURL( 'http://steamcommunity.com/workshop/filedetails/discussion/940672885/1291817837636529927/' )
				end
				btnBug:SetTall( 25 )
				pnl:AddItem( btnBug )
				...
				btnRate.DoClick = function()
					gui.OpenURL( 'http://steamcommunity.com/sharedfiles/filedetails/?id=940672885' )
				end
				btnRate:SetTall( 50 )
				pnl:AddItem( btnRate )
				...
				pnl:Help( 'Ability: Lightning Bolt' )
				pnl:Dock( FILL )
				...
				if LocalPlayer():IsAdmin() then
					...
					pnl:CheckBox( 'Spawnable SWEP', 'sk_mas_bloodmagessoul_enabled' )
					pnl:ControlHelp( 'Console variant: sk_mas_bloodmagessoul_enabled. Note: This will take effect on map change. It may still be spawned through the give weapon command.' )
					pnl:CheckBox( 'Admin Spawnable only', 'sk_mas_bloodmagessoul_adminonly' )
					pnl:ControlHelp( 'Console variant: sk_mas_bloodmagessoul_adminonly. Note: Map change, or you must die / drop / strip it & spawn it again in order for this to take effect' )
					pnl:CheckBox( 'Cheat: Infinite Ultimate Charge' , 'sk_mas_bloodmagessoul_infinitecharge' )
					pnl:ControlHelp( 'Console variant: sk_mas_bloodmagessoul_infinitecharge. Note: Change is immediate.' )
					pnl:CheckBox( 'Cheat: No Cooldowns', 'sk_mas_bloodmagessoul_nocooldown' )
					pnl:ControlHelp( 'Console variant: sk_mas_bloodmagessoul_infinitecharge. Note: You must die/ drop / strip it & spawn it again in order for this to take effect.' )
					pnl:ControlHelp( '' )
