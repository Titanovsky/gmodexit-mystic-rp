...
if ( SERVER ) then
	...
	function MakeHoverboard( pl, model, ang, pos, mcontrol, shake, height, viewdist, trailsize, trail, boost, recharge, attributes )
		...
		attributes.speed = math.Clamp( attributes.speed, 1, 256 )
		attributes.jump = math.Clamp( attributes.jump, 0, 256 )
		attributes.turn = math.Clamp( attributes.turn, 1, 256 )
		attributes.flip = math.Clamp( attributes.flip, 1, 256 )
		attributes.twist = math.Clamp( attributes.twist, 1, 256 )
...
function TOOL.BuildCPanel( cp )
	...
	cp:AddControl( "Slider", { Label = "Hover Height", Min = 72, Max = 72, Command = "hoverboard_height" } ):GetParent():DockPadding( 10, 0, 10, 0 )
